...
export function useAcceptanceAccess() {
  ...
  function canOperateAcceptance(type: 'INTERNAL' | 'CUSTOMER', isAcceptedProjectManager = false, hasInternalQualification = false) {
    if (type === 'INTERNAL') {
      // 资格由 readInternalAcceptanceAccess 核验：运维主管或管理员，普通成员不可办理。
      return hasInternalQualification && hasPermission('project:internalAccept');
    }

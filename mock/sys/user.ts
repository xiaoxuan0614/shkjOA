import { MockMethod } from 'vite-plugin-mock';
import { resultError, resultSuccess, getRequestToken, requestParams, sysUrl } from '../_util';
import { PageEnum } from '/@/enums/pageEnum';
import CryptoJS from 'crypto-js';

// AES CBC 解密（与前端 src/utils/cipher.ts 保持一致）
const AES_KEY = '1234567890adbcde';
const AES_IV = '1234567890hjlkew';
function decryptAES(cipherText: string): string {
  try {
    const bytes = CryptoJS.AES.decrypt(cipherText, CryptoJS.enc.Utf8.parse(AES_KEY), {
      iv: CryptoJS.enc.Utf8.parse(AES_IV),
      mode: CryptoJS.mode.CBC,
      padding: CryptoJS.pad.Pkcs7,
    });
    return bytes.toString(CryptoJS.enc.Utf8);
  } catch (e) {
    // 解密失败则返回原文（兼容明文密码）
    return cipherText;
  }
}
export function createFakeUserList() {
  return [
    {
      userId: '1',
      id: '1',
      username: 'admin',
      realname: '管理员',
      departName: '工程部',
      avatar: 'https://q1.qlogo.cn/g?b=qq&nk=190848757&s=640',
      desc: 'manager',
      password: '123456',
      token: 'fakeToken1',
      homePath: PageEnum.BASE_HOME,
      roles: [
        {
          roleName: 'Super Admin',
          value: 'super',
        },
      ],
    },
    {
      userId: '2',
      id: '2',
      username: 'jeecg',
      password: '123456',
      realname: '测试用户',
      departName: '工程部',
      avatar: 'https://q1.qlogo.cn/g?b=qq&nk=339449197&s=640',
      desc: 'tester',
      token: 'fakeToken2',
      homePath: PageEnum.BASE_HOME,
      roles: [
        {
          roleName: 'Tester',
          value: 'test',
        },
      ],
    },
  ];
}

/**
 * 数据字典：不再维护 mock 字典项（统一取正式后端 sys_dict 数据字典，如 purchase_order_status 等）。
 * 前端 loadDictMap 优先读登录/用户信息返回的 sysAllDictItems（此处为空）→ 实时调 /sys/dict/getDictItems/{code}。
 */
function createDictItems() {
  return {};
}

export default [
  // mock user login
  {
    url: `${sysUrl}/sys/login`,
    timeout: 200,
    method: 'post',
    response: ({ body }) => {
      const { username, password } = body;
      // 兼容 AES 加密密码
      const decryptedPassword = decryptAES(password || '');
      const checkUser = createFakeUserList().find(
        (item) => item.username === username && (password === item.password || decryptedPassword === item.password)
      );
      if (!checkUser) {
        return resultError('Incorrect account or password！');
      }
      const { userId, username: _username, token, realname, desc, roles, departName, id } = checkUser;
      return resultSuccess({
        roles,
        userId,
        username: _username,
        token,
        realname,
        desc,
        // 对齐接口文档登录响应：result.userInfo(含 id/departName/departIds) + result.departs[]（前端取当前用户部门/申请人ID）
        userInfo: { id: id || userId, username: _username, realname, departName, orgCode: 'A01', departIds: '1001' },
        departs: [{ id: '1001', orgCode: 'A01', departName: departName || '工程部' }],
        sysAllDictItems: createDictItems(),
      });
    },
  },
  {
    url: `${sysUrl}/sys/user/getUserInfo`,
    method: 'get',
    response: (request: requestParams) => {
      const token = getRequestToken(request);
      if (!token) return resultError('Invalid token');
      const checkUser = createFakeUserList().find((item) => item.token === token);
      if (!checkUser) {
        return resultError('The corresponding user information was not obtained!');
      }
      return resultSuccess({ userInfo: checkUser, sysAllDictItems: createDictItems() });
    },
  },
  {
    // 用户列表(负责人/对接人选择器数据源)
    url: `${sysUrl}/sys/user/list`,
    method: 'get',
    response: ({ query }) => {
      const { pageNo = 1, pageSize = 1000, realname, keyword } = query;
      let data = createFakeUserList().map((u) => ({ id: u.id, username: u.username, realname: u.realname }));
      if (realname) data = data.filter((u) => (u.realname || '').indexOf(realname) !== -1);
      if (keyword) data = data.filter((u) => (u.realname || '').indexOf(keyword) !== -1);
      const offset = (Number(pageNo) - 1) * Number(pageSize);
      return resultSuccess({ records: data.slice(offset, offset + Number(pageSize)), total: data.length });
    },
  },
  {
    url: `${sysUrl}/sys/permission/getPermCode`,
    timeout: 200,
    method: 'get',
    response: (request: requestParams) => {
      const token = getRequestToken(request);
      if (!token) return resultError('Invalid token');
      const checkUser = createFakeUserList().find((item) => item.token === token);
      if (!checkUser) {
        return resultError('Invalid token!');
      }
      // 直接返回全部权限，页面v‑auth放行，页面代码完全不动
      return resultSuccess([
        '1000',
        '3000',
        '5000',
        'materiallist:add',
        'materiallist:edit',
        'materiallist:delete',
        'materiallist:deleteBatch',
        'materiallist:exportXls',
        'materiallist:importExcel',
        // 物料管理(goods)按钮权限
        'mtl:goods:add',
        'mtl:goods:edit',
        'mtl:goods:delete',
        'mtl:goods:deleteBatch',
        'mtl:goods:io',
        'mtl:goods:importExcel',
        // 项目管理(project)生命周期流转按钮权限(角色顺序权限)
        'project:plan',
        'project:implement',
        'project:internalAccept',
        'project:accept',
        'project:warranty',
      ]);
    },
  },
  {
    url: `${sysUrl}/sys/logout`,
    timeout: 200,
    method: 'get',
    response: (request: requestParams) => {
      const token = getRequestToken(request);
      if (!token) return resultError('Invalid token');
      const checkUser = createFakeUserList().find((item) => item.token === token);
      if (!checkUser) {
        return resultError('Invalid token!');
      }
      return resultSuccess(undefined, { message: 'Token has been destroyed' });
    },
  },
  {
    url: new RegExp(`${sysUrl}/sys/randomImage/.*`),
    timeout: 200,
    method: 'get',
    response: (_request: requestParams) => {
      const result =
        'data:image/jpg;base64,';
      return resultSuccess(result);
    },
  },
  {
    // 默认密码校验：返回 message 含 'yes' 会弹修改默认密码框 → mock 返回 'no'
    url: `${sysUrl}/sys/user/verifyIzDefaultPwd`,
    timeout: 200,
    method: 'get',
    response: () => ({ success: true, message: 'no', code: 0, result: null, timestamp: Date.now() }),
  },
  {
    url: `${sysUrl}/sys/loginGetUserDeparts`,
    timeout: 200,
    method: 'post',
    response: (_request: requestParams) => {
      return resultSuccess({
        departs: [],
        currentOrgCode: '',
      });
    },
  },
  {
    url: `${sysUrl}/sys/annountCement/getUnreadMessageCount`,
    timeout: 200,
    method: 'get',
    response: (_request: requestParams) => {
      return resultSuccess(0);
    },
  },
  {
    url: `${sysUrl}/sys/annountCement/listByUser`,
    timeout: 200,
    method: 'get',
    response: (_request: requestParams) => {
      return resultSuccess({
        records: [],
        total: 0,
      });
    },
  },
] as MockMethod[];
